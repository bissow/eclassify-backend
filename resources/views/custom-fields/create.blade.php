                                    ...
                                    @foreach ($categories as $category)
                                        <div class="category">
                                            <div class="category-header">
                                                <label>
                                                    <input type="checkbox" name="selected_categories[]" value="{{ $category->id }}" class="category-checkbox"> {{ $category->name }}
                                                </label>
...
@section('js')
    <script>
        ...
        $(document).on('click', '.category-checkbox', function(e) {
            ...
            // 1. Select/deselect all child categories automatically (UI only)
            $categoryDiv.find('.subcategories .category-checkbox').prop('checked', isChecked);

            // 2. Select parent if all siblings are checked, deselect if not
            updateParentCheckboxes($(this));
        });

        function updateParentCheckboxes($checkbox) {
            const $subcategoriesDiv = $checkbox.closest('.subcategories');
            if ($subcategoriesDiv.length > 0) {
                const $parentCategoryDiv = $subcategoriesDiv.parent();
                const $parentCheckbox = $parentCategoryDiv.children('.category-header').find('.category-checkbox');

                if ($parentCheckbox.length > 0) {
                    const totalSiblings = $subcategoriesDiv.find('> .category > .category-header .category-checkbox').length;
                    const checkedSiblings = $subcategoriesDiv.find('> .category > .category-header .category-checkbox:checked').length;

                    const shouldCheckParent = (totalSiblings > 0 && totalSiblings === checkedSiblings);
                    $parentCheckbox.prop('checked', shouldCheckParent);
