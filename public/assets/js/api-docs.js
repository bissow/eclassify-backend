(function () {
    ...
    function syncActiveCardFromHash() {
        ...
        const found = endpointList.find(ep => {
            if (ep.operationId && hash.includes(ep.operationId.toLowerCase())) {
                return true;
            }
            const cleanPath = ep.path.toLowerCase().replace(/[^a-z0-9]/g, '');
            const cleanMethod = ep.method.toLowerCase();
            return hash.includes(cleanMethod) && hash.includes(cleanPath);
        });
    ...
    function createApiInfoPanel(ep) {
        ...
        if (isGetLike) {
            ...
            bodyFields.forEach(f => {
                ...
                const meta = formatSchemaMeta(f.schema || f);
                paramsHtml += `
                    <div class="param-doc-item">
                        <div class="param-doc-item-row">
    ...
    function createApiCard(method, path, operation, responseBoxEl) {
        ...
        headersState.push({ key: 'Content-Language', value: 'en', fixed: false, included: true });
        headersState.push({ key: 'Authorization', value: '', fixed: false, included: true });
        if (!isGetLike) {
