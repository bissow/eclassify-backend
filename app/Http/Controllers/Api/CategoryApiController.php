    ...
    public function getSubCategories(Request $request)
    {
        ...
        try {
            ...
            if (! empty($categoryIds)) {
                // Batch-fetch ancestor + descendant IDs in two queries instead of 2N
                $categories = Category::whereIn('id', $categoryIds)->get();

                $allRelatedIds = [];
                foreach ($categories as $cat) {
                    $ancestorIds   = $cat->ancestors()->where('status', 1)->pluck('id')->toArray();
                    $descendantIds = $cat->descendants()->where('status', 1)->pluck('id')->toArray();
            ...
            if ($parentCategory !== null) {
                $parentCat        = Category::find($parentCategory->id);
                $ancestorIds      = $parentCat->ancestors()->where('status', 1)->pluck('id')->toArray();
                $selfAndAncestorsIds = array_unique(array_merge([$parentCat->id], $ancestorIds));
